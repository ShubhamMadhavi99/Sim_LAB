...
% It calculates the time delays, attenuation factors, and performs FFT analysis of the received signal.
...
d2 = 2 * sqrt((d/2)^2 + h1^2);  % Distance due to ionospheric reflection
...
a2 = (1 / d2) * 0.5;   % Ionospheric reflection (R = 0.5)
...
%% FFT Calculation
fs = 1668e6;  % Sampling frequency, considering fm = 834MHz and fs = 2*fm
q = fft(Y);
n = length(q);
Q = abs(q);
